...
public static partial class AWSBedrock
{
    [GeneratedRegex(@"[^a-z]")]
    private static partial Regex SanitizeMetaLlamaRegex();
    [GeneratedRegex(@"\(([a-zA-Z]+)\)")]
    private static partial Regex SanitizeAnthropicClaudeRegex();
    ...
    public static string Sanitize(string text) =>
        FoundationalModel == FoundationalModel.META_LLAMA3
            ? SanitizeMetaLlamaRegex().Replace(text, "")
            : FoundationalModel == FoundationalModel.ANTHROPIC_CLAUDE1
                ? SanitizeAnthropicClaudeRegex().Match(text).Groups[1].Value
                : text;
